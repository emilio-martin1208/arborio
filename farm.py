...
ANIMAL_IMAGES = {"rabbit": load_image("Rabbit.png"), "bird": load_image("Bird.png"),
                  ...
                  "goat": load_image("Goat.png"), "dog": load_image("Dog.png"), "pig": load_image("Pig.png"),
                  "wild_horse": load_image("WildHorse.png"), "donkey": load_image("Donkey.png")}
...
ANIMAL_SPEED = {"rabbit": 1.3, "bird": 1.8, "fox": 1.6, "deer": 1.5, "hedgehog": 0.7, "squirrel": 1.7,
                 ...
                 "goat": 1.1, "dog": 1.6, "pig": 0.8, "wild_horse": 1.7, "donkey": 1.2}
...
def draw_landmark(screen, kind, cx, cy, tile_draw_size, discovered):
    ...
    elif kind == "abandoned_campsite":
        ring_r = tile_draw_size * 0.28
        pygame.draw.circle(screen, (90, 60, 40), (int(cx), int(cy)), int(ring_r), max(1, int(tile_draw_size * 0.04)))
        for i in range(5):
            ang = i * (2 * math.pi / 5)
            log_x = cx + math.cos(ang) * ring_r * 0.7
            log_y = cy + math.sin(ang) * ring_r * 0.7
            pygame.draw.line(screen, (60, 40, 28), (cx, cy), (log_x, log_y), max(1, int(tile_draw_size * 0.05)))
    elif kind == "wagon_remains":
...
def draw_idle_action(screen, species, action, cx, cy, tile_draw_size, ticks):
    ...
    if species == "chicken" and action == "peck":
    ...
    elif species == "chicken" and action == "dust_bathe":
        for i in range(3):
            ang = ticks * 0.006 + i * 2.1
            r = tile_draw_size * (0.15 + 0.05 * i)
            dx = cx + math.cos(ang) * r
            dy = cy + tile_draw_size * 0.3 + math.sin(ang) * r * 0.4
            pygame.draw.circle(screen, (210, 195, 160), (int(dx), int(dy)), 2)
    elif species == "pig" and action == "mud_roll":
        wobble = math.sin(ticks * 0.015) * tile_draw_size * 0.08
        pygame.draw.ellipse(screen, (90, 70, 50), (int(cx - tile_draw_size * 0.3 + wobble),
                                                     int(cy + tile_draw_size * 0.3),
                                                     int(tile_draw_size * 0.6), int(tile_draw_size * 0.15)))
    elif species == "pig" and action == "truffle_sniff":
        sniff_bob = abs(math.sin(ticks * 0.025)) * tile_draw_size * 0.08
        pygame.draw.circle(screen, (140, 100, 70),
                            (int(cx + tile_draw_size * 0.25), int(cy + tile_draw_size * 0.3 + sniff_bob)), 2)
    elif species == "sheep" and action == "rub":
    ...
    elif species == "rabbit" and action == "stand":
        ...
        pygame.draw.line(screen, (232, 176, 178), (cx - tile_draw_size * 0.1, cy - tile_draw_size * 0.1),
                          (cx - tile_draw_size * 0.1 + lick, cy - tile_draw_size * 0.2), 2)
    elif action == "sleep":
    ...
    elif species == "cow" and action == "chew_cud":
    ...
    elif species == "goat" and action == "climb":
        #A little lift, as if perched up on whatever's nearby.
        pygame.draw.line(screen, (150, 148, 140), (cx - tile_draw_size * 0.2, cy + tile_draw_size * 0.4),
                          (cx + tile_draw_size * 0.2, cy + tile_draw_size * 0.4), 3)
    elif species == "goat" and action == "headbutt":
        lunge = abs(math.sin(ticks * 0.03)) * tile_draw_size * 0.15
        pygame.draw.line(screen, (140, 118, 90), (cx + tile_draw_size * 0.2, cy),
                          (cx + tile_draw_size * 0.2 + lunge, cy), 3)
    elif species == "goat" and action == "nibble":
        #Goats famously nibble at whatever's around, not just one thing.
...
spawn_collectible_scattered("truffle", 18, biomes=("jungle",))
spawn_collectible_scattered("lucky_rock", 25, biomes=("tundra", "desert"))
spawn_collectible_scattered("fossil", 20, biomes=("desert",))
